...
insert_feed(const struct string *feed_url, struct feed_bucket *feed)
{
	...
	sqlite3_bind_text(s, FEED_COLUMN_GENERATOR    + 1, feed->generator->ptr,    feed->generator->len,    NULL);
	sqlite3_bind_text(s, FEED_COLUMN_RIGHTS       + 1, feed->rights->ptr,       feed->rights->len,       NULL);
